    ...
    async execute(interaction, args, database, bot) {
        ...
        switch (args[0].name) {
            case "lines":
                ...
                let scriptString = "";
                let lineCount = 0;
                const nickname = args[0].options[2] ? args[0].options[2].value : bot.guilds.cache.get(interaction.guild_id).members.cache.get(interaction.member.user.id).nickname;
                let temp = "";
                ...
                if (scriptString !== "") Embed
                    ...
                    .setTitle(`${args[0].options[2] ? `${nickname}'s` : "Your"} line${lineCount > 1 ? "s" : ""} for S${args[0].options[0].value}E${args[0].options[1].value}`)
                    .setColor('RANDOM');

                bot.users.cache.get(interaction.member.user.id).send(scriptString !== "" ? Embed : `You don't have any scripts in S${args[0].options[0].value}E${args[0].options[1].value}!`);
                break;
            case "full":
                try {
